...
export const LifestyleQuestionnaireFormScreen: React.FC<Props> = ({
  ...
  const [hasReadConsent, setHasReadConsent] = useState(isConsentCompleted);
  const [isConsentChecked, setIsConsentChecked] = useState(isConsentCompleted);
  ...
  const handleConsentScroll = () => {
    ...
    if (!element) return;
    if (element.scrollHeight - element.scrollTop - element.clientHeight <= 24) setHasReadConsent(true);
  };

  const handleConsentNext = () => {
    if (!isConsentChecked || !hasReadConsent) return;
    if (!isConsentCompleted) onConsentComplete?.();
    setCurrentPage(2);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white relative font-sans antialiased text-slate-900 select-none">
      ...
      <div ref={currentPage === 1 ? consentScrollRef : undefined} onScroll={currentPage === 1 ? handleConsentScroll : undefined} className="flex-1 overflow-y-auto bg-white p-5">
        {currentPage === 1 ? (
          /* PAGE 1: 沿用問卷版型呈現個人資料利用暨授權同意書 */
          <div className="space-y-4 pb-4">
            <h2 className="text-lg font-black text-slate-900">個人資料利用暨授權同意書</h2>
            <div className="space-y-4 text-sm text-slate-700 leading-7">
              <p>感謝您使用「WaCare」。吉樂健康資訊科技股份有限公司（以下簡稱「吉樂健康」）參與台灣家庭醫學醫學會（以下簡稱「家醫學會」）所執行之「健康台灣深耕計畫–綠色家庭醫學推動計畫–結合智慧科技健康促進處方訓練，由家庭醫師實踐 ESG 導向永續健康創新照護模式」（以下簡稱「本計畫」）。「WaCare」之隱私權政策，同樣適用於本計畫，並無變更；惟若您選擇參與本計畫，尚需另行同意以下關於資料去識別化後提供予家醫學會之利用方式。</p>
              <p>您同意吉樂健康就與健康管理相關之資料進行去識別化處理，該等資料包含但不限於健康量測數據（如體重、血壓、血糖、身高、運動紀錄等）、生活型態資料（如飲水、飲食、步數等）、與健康促進相關之行為或生活型態資料，以及綠色處方資料等。前述資料經吉樂健康完成去識別化後，將提供給家醫學會用於提供健康促進相關服務（包含但不限於生活型態建議與健康管理）、進行資料分析與統計、作為本計畫執行與成效評估之依據，並供研究及政策規劃參考使用。</p>
              <p>您並同意吉樂健康將前述經去識別化之資料提供予家醫學會使用。家醫學會僅接收並利用無法識別特定個人身分之去識別化資料，作為統計分析、研究及計畫成效評估之用；該等資料不致識別特定個人身分，且無法回復識別。前述資料之利用期間為本計畫執行期間（含展延期間）。</p>
              <p>當您勾選「我已閱讀並同意」並簽署時，表示您已詳細閱讀本同意書，瞭解並同意吉樂健康依據本同意書之內容，對您的個人資料進行去識別化處理及相關利用，並提供予家醫學會使用。您得自由選擇是否提供個人資料，惟若拒絕提供或同意，您將無法參與本計畫所提供之智慧科技健康促進處方及相關健康管理服務。</p>
            </div>
            <label className={`flex items-center gap-3 rounded-xl border p-3 ${hasReadConsent ? 'cursor-pointer border-orange-200 bg-orange-50/60' : 'cursor-not-allowed border-slate-200 bg-slate-50 opacity-60'}`}><input type="checkbox" disabled={!hasReadConsent} checked={isConsentChecked} onChange={(event) => setIsConsentChecked(event.target.checked)} className="h-5 w-5 rounded border-slate-300 text-[#f37021] focus:ring-orange-500" /><span className="text-sm font-bold text-slate-800">我已閱讀並接受上述同意書內容</span></label>
            {!hasReadConsent && <p className="text-xs font-bold text-orange-700">請將同意書內容滑動閱讀至最底部，才可勾選同意。</p>}
          </div>
        ) : (
      ...
      <footer className="p-4 bg-white border-t border-slate-100 sticky bottom-0 z-20 shadow-lg">
        {currentPage === 1 ? (
          ...
          <button
            type="button"
            disabled={!hasReadConsent || !isConsentChecked}
            onClick={handleConsentNext}
            className={`w-full py-3.5 font-black text-base rounded-2xl shadow-sm transition-all flex items-center justify-center gap-1.5 ${hasReadConsent && isConsentChecked ? 'bg-[#f37021] hover:bg-orange-600 active:scale-98 text-white cursor-pointer' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
          >
            <span>下一頁</span>
          </button>
        ) : (
