...
import { HistoricalSnapshot, getPeriodEnd } from './greenPrescriptionHistory';

const DEMO_HISTORY_VIDEO_SNAPSHOT = ['正確量血壓的方法', '認識高血壓飲食原則', '建立每日運動習慣'].map((title, index) => ({ id: `history-video-2026-08-03-${index + 1}`, title }));
const QUESTIONNAIRE_REMINDER_DAYS = 30;
const parseQuestionnaireCompletedAt = (completedAt: string): number => {
  const normalized = completedAt.replace(/\//g, '-').replace(' ', 'T');
...
export const GreenPrescriptionDashboard: React.FC<Props> = ({
  ...
  const [activeMainTab, setActiveMainTab] = useState<'overview' | 'videos' | 'questionnaire' | 'prescriptions' | 'history'>('overview');
  ...
  const hasQuestionnaire = isQuestionnaireSubmitted;
  const latestQuestionnaireCompletedAt = questionnaireHistory.reduce(
    (latest, record) => Math.max(latest, parseQuestionnaireCompletedAt(record.completedAt)),
    0,
  );
  const shouldPromoteQuestionnaire = latestQuestionnaireCompletedAt === 0
    || Date.now() - latestQuestionnaireCompletedAt > QUESTIONNAIRE_REMINDER_DAYS * 24 * 60 * 60 * 1000;
  ...
  // Persisted prescription definitions may exist from a previous session, but
  // they are not current assignments until the backend dispatch flag is true.
  const assignedSections = isPrescriptionDispatched
    ? Object.values(prescriptionData) as DoctorPrescriptionSection[]
    : [];
  ...
  const greenVideoCategories = [
    { key: 'diet', label: '飲食', words: ['飲食'] },
    { key: 'activity', label: '身體活動', words: ['運動', '身體活動'] },
    { key: 'sleep', label: '睡眠', words: ['睡眠'] },
    { key: 'stress', label: '壓力管理', words: ['壓力'] },
  ...
  // MAIN VIEW: 綠色處方主儀表板
  // -------------------------------------------------------------
  if (activeMainTab === 'overview' || activeMainTab === 'videos' || activeMainTab === 'questionnaire' || activeMainTab === 'prescriptions' || activeMainTab === 'history') {
    const prescriptionGroups = Array.from(assignedSections.reduce((map, section) => {
      const existing = map.get(section.categoryTitle);
      if (existing) existing.items = [...existing.items, ...section.items];
      else map.set(section.categoryTitle, { ...section, items: [...section.items] });
      return map;
    }, new Map<string, typeof assignedSections[number]>()).values()).filter((section) => section.items.length > 0);
    const pendingPrescriptionGroups = prescriptionGroups.map((section) => ({ ...section, items: section.items.filter((item) => !item.completed) })).filter((section) => section.items.length > 0);
    ...
    const questionnaireOverviewButton = <button type="button" data-questionnaire-reminder-due={String(shouldPromoteQuestionnaire)} onClick={() => handleGoToQuestionnaire('form')} className="flex w-full items-center justify-between rounded-2xl border-2 border-orange-400 bg-orange-50/60 p-4 text-left shadow-sm"><span><span className="block font-black text-slate-900">生活型態問卷<span className="text-orange-700">（必填）</span></span><span className="mt-1 block text-sm font-bold text-slate-700">{isDoctorAssigned ? '如果想要調整關注類型，請重新填寫問卷並與醫師討論' : hasQuestionnaire ? '問卷已完成，等待專家指派處方。' : '請先填寫完問卷，醫師才能依照內容為您指派處方。'}</span></span><ChevronRight className="h-5 w-5 text-orange-600" /></button>;
    return <div className="flex h-full min-h-0 flex-col bg-slate-50 text-slate-900" data-dashboard-layout="task-center" data-prescription-group-count={prescriptionGroups.length} data-prescription-item-count={prescriptionItemCount} data-prescription-unique-composite-id-count={uniquePrescriptionItemCount} data-questionnaire-history-count={questionnaireHistory.length}>
      ...
      <main onClick={(event) => { const target = (event.target as HTMLElement).closest('[data-history-period-id]'); if (target) setExpandedHistoryId(target.getAttribute('data-history-period-id')); }} className="min-h-0 flex-1 overflow-y-auto p-4 pb-10">
        {activeMainTab === 'overview' && <section className="space-y-5">
          <h2 className="text-base font-black">本期任務清單</h2>
          {shouldPromoteQuestionnaire && questionnaireOverviewButton}
          {isDoctorAssigned && <section className="rounded-2xl border border-orange-200 bg-white p-4"><div className="flex items-center justify-between"><div><h3 className="font-black">專家處方</h3><p className="mt-1 text-sm text-slate-600">已完成 {greenPrescriptionProgress.prescriptionCompleted} / {greenPrescriptionProgress.prescriptionTotal}，尚有 {Math.max(0, greenPrescriptionProgress.prescriptionTotal - greenPrescriptionProgress.prescriptionCompleted)} 項待完成</p></div><button type="button" onClick={() => setActiveMainTab('prescriptions')} className="text-xs font-bold text-orange-600">查看全部</button></div>{pendingPrescriptionGroups.map((group) => <div key={group.categoryTitle} className="mt-4 border-t border-slate-100 pt-3"><h4 className="text-sm font-black">{group.categoryTitle}</h4>{group.items.map((item) => <button key={item.id} type="button" data-overview-prescription-task-id={item.id} onClick={() => onTogglePrescriptionItem(group.id, item.id)} className="mt-2 flex w-full items-center gap-2 text-left text-sm text-slate-700"><span className="h-5 w-5 shrink-0 rounded border border-slate-300" />{item.title}</button>)}</div>)}</section>}
          <section className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><span><span className="block font-black">推薦處方影片</span><span className="mt-1 block text-sm text-slate-600">已完成 {greenPrescriptionProgress.videoCompleted} / {greenPrescriptionProgress.videoTotal}，尚需 {Math.max(0, greenPrescriptionProgress.videoTotal - greenPrescriptionProgress.videoCompleted)} 部</span></span><ChevronRight className="h-5 w-5 text-slate-400" /></div><div className="mt-3 space-y-2">{tasks.filter((task) => task.type === 'video').slice(0, 5).map((video) => <button key={video.id} type="button" onClick={() => setActiveVideoId(video.id)} className="flex w-full items-center gap-2 text-left text-sm font-bold text-slate-700"><span>{video.completed ? '✓' : '▶'}</span>{video.title}</button>)}</div></section>
        </section>}
        ...
        {activeMainTab === 'history' && <section className="space-y-4"><h2 className="text-base font-black">過往執行紀錄</h2>{historicalSnapshots.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-6 text-center text-sm text-slate-500">目前尚無過往執行紀錄</div> : historicalSnapshots.map((snapshot) => { const tasksForSnapshot = snapshot.prescriptionTaskSnapshot ?? []; const prescriptionTotal = tasksForSnapshot.length; const prescriptionCompleted = tasksForSnapshot.filter((task) => task.completed).length; const overallTotal = prescriptionTotal + snapshot.videoTotal; const overallCompleted = prescriptionCompleted + snapshot.videoCompleted; const incomplete = overallTotal - overallCompleted; const percentage = calculateProgressPercent(overallCompleted, overallTotal); const expanded = expandedHistoryId === snapshot.id; const completedVideoNames = DEMO_HISTORY_VIDEO_SNAPSHOT.slice(0, Math.min(snapshot.videoCompleted, snapshot.videoTotal)); const incompleteVideoCount = Math.max(0, snapshot.videoTotal - snapshot.videoCompleted); return <div key={snapshot.id} role="button" tabIndex={0} onClick={() => setExpandedHistoryId(expanded ? null : snapshot.id)} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between"><div><p className="font-bold">{snapshot.startDate}－{snapshot.endDate}</p><p className="mt-3 text-sm font-bold">整體達成率 {percentage}%</p><p className="mt-1 text-sm text-slate-600">已完成 {overallCompleted} 項｜未完成 {incomplete} 項</p><div className="mt-3 grid grid-cols-2 gap-2 text-sm font-bold"><span>專家處方 {prescriptionCompleted} / {prescriptionTotal}</span><span>課程 {snapshot.videoCompleted} / {snapshot.videoTotal}</span></div></div><div className="flex flex-col items-end gap-2">{snapshot.expertName ? <span className="text-[11px] font-bold text-emerald-600 whitespace-nowrap">由{snapshot.expertName}指派</span> : <span className="text-[11px] font-bold text-amber-600 whitespace-nowrap">未收到專家指派處方</span>}{expanded ? <ChevronUp className="h-5 w-5 text-slate-400" /> : <ChevronDown className="h-5 w-5 text-slate-400" />}</div></div>{expanded && <div className="mt-4 border-t border-slate-100 pt-3"><p className="text-sm font-bold">專家處方 {prescriptionCompleted} / {prescriptionTotal}</p>{Array.from(new Set(tasksForSnapshot.map((task) => task.category))).map((category) => <div key={category} className="mt-3"><p className="text-xs font-black text-slate-600">{historyCategoryLabel(String(category))}</p>{[...tasksForSnapshot.filter((task) => task.category === category && !task.completed), ...tasksForSnapshot.filter((task) => task.category === category && task.completed)].map((task) => <p key={task.id} className="mt-1 text-xs text-slate-600">{task.completed ? '✓' : '✕'} {task.title}</p>)}</div>)}<p className="mt-4 text-sm font-bold">課程 {snapshot.videoCompleted} / {snapshot.videoTotal}</p>{completedVideoNames.map((video) => <p key={video.id} className="mt-1 text-xs text-slate-600">✓ {video.title}</p>)}{incompleteVideoCount > 0 && <p className="mt-1 text-xs text-slate-600">未達成觀看影片 {incompleteVideoCount} 部</p>}</div>}</div>; })}</section>}
      </main>
    </div>;
  }
  return (
