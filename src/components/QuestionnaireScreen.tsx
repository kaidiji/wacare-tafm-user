...
export const QuestionnaireScreen: React.FC<Props> = ({
  ...
  const [showLifestyleForm, setShowLifestyleForm] = useState(initialView === 'form');
  ...
  const completedList: QuestionnaireRecord[] = questionnaireHistory.map((record) => ({
          id: record.id,
          title: '生活型態問卷',
  ...
  const monthGroups = currentList.reduce<Array<{ key: string; label: string; items: QuestionnaireRecord[] }>>(
    (groups, item) => {
      ...
      const month = dateMatch?.[2] || String(now.getMonth() + 1);
      const key = `${year}-${month.padStart(2, '0')}`;
      const existingGroup = groups.find((group) => group.key === key);
      if (existingGroup) existingGroup.items.push(item);
      else groups.push({ key, label: `${year} 年 ${Number(month)} 月`, items: [item] });
      return groups;
    },
    []
  );
  ...
  const handleFinishLifestyleForm = (goals: string[]) => {
    ...
    if (onDirectSubmitComplete) {
      onDirectSubmitComplete();
    }
    // 從問卷燈進入時，App 會因歷史筆數更新而重新建立本畫面並顯示完成清單。
    // 此處不可再更新舊表單的 local state，否則舊 Props 會覆蓋剛建立的新清單。
  };
  ...
  if (showResultScreen) {
    return (
      <QuestionnaireResultScreen
        onClose={() => initialView === 'result' ? onBack() : setShowResultScreen(false)}
        title="生活型態問卷"
        dateLabel={(selectedRecord?.completedDate || questionnaireHistory[0]?.completedAt || '').split(' ')[0]}
        completedDate={selectedRecord?.completedDate || questionnaireHistory[0]?.completedAt || ''}
        submittedGoals={selectedRecord?.goals || questionnaireHistory[0]?.goals || submittedGoals}
      />
    );
  }
        ...
        {currentList.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col bg-white">
            {monthGroups.map((group) => {
              const isCollapsed = collapsedMonths.includes(group.key);
              return <div key={group.key}>
                <button type="button" onClick={() => toggleMonth(group.key)} className="w-full px-5 py-4 flex items-center justify-between border-b border-slate-100 bg-white cursor-pointer select-none">
                  <h2 className="text-[1.0625rem] font-black text-slate-900 tracking-tight">{group.label}</h2>
